export default {
  ...
  actions: {
    ...
    getCommunitys(context) {
      this.$axios.get('/board/community').then(({ data }) => {
        console.log('가져오기성공ㅇ');
    ...
    addCommunity(context, payload) {
      this.$axios.post('/board/community', payload).then(({ data }) => {
        console.log('등록성공');
        this.$axios.get('/board/community').then(({ data }) => {
          console.log('가져오기성공ㅇ');
          context.commit('setCommunitys', data);
    ...
    modifyCommunity(context, payload) {
      this.$axios.put('/board/community', payload).then(({ data }) => {
        console.log('수정성공');
      });
    },
    // 커뮤니티 삭제
    deleteCommunity(context, payload) {
      this.$axios.delete('/board/community' + payload).then(({ data }) => {
        console.log('삭제성공');
      });
    },
  },
